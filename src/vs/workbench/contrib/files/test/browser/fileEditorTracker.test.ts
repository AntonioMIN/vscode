...
suite('Files - FileEditorTracker', () => {
	...
	async function createTracker(): Promise<[EditorPart, ServiceAccessor, FileEditorTracker, IInstantiationService]> {
		const instantiationService = workbenchInstantiationService();
		...
		return [part, accessor, tracker, instantiationService];
	}
	...
	test('dirty untitled text file model opens as editor', async function () {
		const [part, accessor, tracker, instantiationService] = await createTracker();

		const untitledEditor = instantiationService.createInstance(UntitledTextEditorInput, accessor.textFileService.untitled.create());
